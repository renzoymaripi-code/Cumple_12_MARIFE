...
const combinationLock =
    document.getElementById("combinationLock");

const lockWheels =
    document.querySelectorAll(".lock-wheel");
...
function updateLockWheel(index) {
    ...
    const number =
        wheel.querySelector(".wheel-number");
...
function rotateWheel(index, direction) {
    ...
    if (
        combinationLock.classList.contains(
            "unlocked"
        )
    ) {
        return;
    ...
    lockValues[index] += direction;


    if (lockValues[index] > 9) {
        lockValues[index] = 0;
    }
    ...
    updateLockWheel(index);

    vibrate(12);

    playTone(
        220 + lockValues[index] * 18,
        0.035,
        "square",
        0.035
    );
...
/* =========================================================
   20. DETECTOR DE PISTAS CON LINTERNA
========================================================= */

const hiddenClues =
    document.querySelectorAll(".hidden-clue");

const discoveredClues =
    new Set();

const FLASHLIGHT_RADIUS = 115;


function detectHiddenClues(x, y) {
    ...
    hiddenClues.forEach((clue, index) => {
        ...
        /*
           La letra aparece únicamente
           cuando está dentro del haz.
        */
        ...
        if (distance < FLASHLIGHT_RADIUS) {
            ...
            /*
               Registramos que la agente
               ya encontró esta letra.
            */

            if (
                !discoveredClues.has(index)
            ) {

                discoveredClues.add(index);
